...
extension CountryCodeViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.backgroundColor = ThemeManager.currentTheme().mainBackgroundColor
        cell.textLabel?.font = UIFont.systemFont(ofSize: 18)
        
        let countryName = filteredCountries[indexPath.row]["name"]!
        let countryDial = " " + filteredCountries[indexPath.row]["dial_code"]!
        
        let countryNameAttribute = [NSAttributedStringKey.foregroundColor: ThemeManager.currentTheme().mainTitleColor]
        let countryDialAttribute = [NSAttributedStringKey.foregroundColor: ThemeManager.currentTheme().mainSubTitleColor]
